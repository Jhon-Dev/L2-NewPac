...
public class AdminSearch implements IAdminCommandHandler
{
   ...
   {
       List<Item> items = Arrays.asList(ItemData.getInstance().getTemplate(page)).stream().filter(item -> item != null && matches(item.getName(), search)).collect(Collectors.toList());
